...
export default class streetElementLink { // Link between two nodes
    constructor (
        ...
        //     new_node.getCoordinates()
        // ]);
        // // Calculates sphere distance, better than lineal this case
        // var l_distance =
        //     Math.round(
        //         (getGeometryLength(l_line)) * 100) / 100;
        // if (l_distance < 120){
        //     //less than 120 meters, simple link
        //     addLink(this.getLastSelectedNode(),
        //             new_node);
        // } else {
        //     // a long distance needs routing
        //     addMultiLink(this.getLastSelectedNode(),
        //                  new_node);
        // }
        ///////////////////////////////////////////////////////////
        ...
        this.lineStringMultiPoint = (nodeA, nodeB) => {
            // a long distance between nodes needs routing
            console.log('addMultiLink');

            var coordinates = this.router.route([
                nodeA.getCoordinates(),
                nodeB.getCoordinates()
            ]);
            ...
            coordinates.then(
                data => {
                    console.log(data);
                    const feature_long = new Feature({
                        geometry: new LineString(data),
                        name: 'MultiLine'

                    });
                    layer.getSource().addFeature(feature_long);
                }
            );
        };
