...
export default class streetElementRoutingMachine {
  constructor(params = {}) {
    var routing_server = "http://161.35.54.122:5000/route/v1/driving/";
    ...
    this.route = async (coordinates) => {
      ...
      return fetch(
        ...
        .then((data) => {
          if (data.code == "Ok") {
            var realcoords = data.routes[0].geometry.coordinates.map((value) =>
              fromLonLat(value)
            );
            return realcoords;
          } else {
  ...
  static isInstance(obj) {
    ...
    console.log(
      "The variable is not a " + streetElementRouter.name + " instance"
    );
    return false;
  }
}
