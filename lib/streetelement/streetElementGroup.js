...
export default class streetElementGroup {
  ...
  constructor(params) {
    ...
    var shapes_router = new streetElementRoutingMachine(); // NOTE Could be random router service
    ///////////////////////////////////////////////////////////
    ...
    var addLayer = (type, color) => {
      ...
      switch (type) {
        case streetElementNode.type.WAYPOINT: // Shape element, blue
          radius = 5;
          style = new Style({
            image: new Circle({
    ...
    this.historyPush = (command) => {
      ...
      if (
        (command[0] == "selectNodeByID") | // Process selectNodeByID
        (command[0] == "unselectNode")
      ) {
        // or unselectNode

        if (
          (history[history.length - 1][0] == "selectNodeByID") |
    ...
    this.getNodeByID = (node_id) => {
      // TODO
      return this.nodes[node_id]; // TODO TEMP
    };
    ...
    this.addNode = (params) => {
      ...
      if (
        ...
        params.type == streetElementNode.type.STOP &&
        this.nodes.some((node) => node.getStopInfo().stop_id == params.stop_id)
      ) {
        return false;
    ...
    this.addShape = (shape_id, shape_segments, params = {}) => {
      // FIXME params update
      ...
      this.shapes.array.push(
        new streetElementShape({
          ...
          waypoints: [],
          node_index: this.getNodeByID,
          router: shapes_router,
          segments: shape_segments, // TODO deprecate
        })
      );
    };
    ...
    this.linkNodesByID = (nodeA_id, nodeB_id) => {
      ...
      addLink(this.nodes[nodeA_id], this.nodes[nodeB_id]);
    };
    ...
    this.changeNodeInfoByID = (node_id, info) => {
      ...
      if (info.type) {
        this.nodes[node_id].setLayer(layers[info.type]);
      }
      ...
      this.nodes[node_id].setStopInfo(info);
...
parent_station,stop_timezone,wheelchair_boarding".split(
    ...
    this.shapesToGTFS = () => {
      ...
      try {
        this.shapes.array.forEach((shape) => {
          ...
          shape.getSegments().forEach((segment) => {
            try {
              var temp_nodes_array = streetElementShape
                .routeSegment(
              ...
              throw (
                ...
                segment[1] +
                "; are valid? " +
                this.nodes[segment[0]].isValid() +
                " : " +
                this.links[segment[1]].isValid()
              );
            }
            temp_nodes_array.pop(); // Drop first element:

            result_nodes_array.push(temp_nodes_array.reverse());
          });
          ...
          result_nodes_array.flat().forEach(
            ...
            (node, key) => {
              ...
              if (key) {
                // if node is not the first index != 0
    ...
    addLayer(streetElementNode.type.WAYPOINT, "#585CCB");
    ...
    map_select_nodes.on("select", (event) => {
      ...
      if (event.deselected.length) {
        if (!event.selected.length) {
          this.map.getOverlayById("popup_node_info").setPosition(undefined);
    ...
    // bundle.o_se_group.map.getLayers().getArray().filter(layer => layer.name == 'waypoint')[0].getSource().getFeatureByUid(9287).parent.terminate()
    ...
    map_translate.on("translateend", (event) => {
      ...
      // refresh all nodes position and log
    ...
    this.map.on("singleclick", (event) => {
      if (mode != "add") {
        // event for add TODO: draw interaction//
  ...
  nodesBetween(nodeA, nodeB) {
    if (
      streetElementNode.isInstance(nodeA) & streetElementNode.isInstance(nodeB)
    ) {
  ...
  focusNodeOnMapByID(node_id) {
    ...
    this.pointer.coordinate = toLonLat(
      this.nodes[node_id].getCoordinates()
    ).map((coor) => coor.toPrecision(8));
  }

  setNodeCoordinatesByID(node_id, coordinates, routed = false) {
    // External
    if (this.nodes[node_id].getLonLat() == coordinates) {
      console.log("no change");
      return;
    }
    this.historyPush(["setNodeCoordinatesByID", node_id, coordinates, routed]);
    this.nodes[node_id].setCoordinates({ lonLat: coordinates, routed: routed });
  }
