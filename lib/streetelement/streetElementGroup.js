...
import {Control, Attribution, FullScreen, ScaleLine, defaults as defaultsControl } from 'ol/control';
import {Select, Translate, defaults as defaultsInteractions} from 'ol/interaction';
    ...
    constructor (center=[0.0,0.0], constraintArea) {
        ...
        this.addNode = (params) => {
            ...
            if (this.getLastSelectedNode()){ // Connect nodes
                try {
                    addLink(this.getLastSelectedNode(),
                            new_node
                           );
                } catch (err) {
...
parent_station,stop_timezone,wheelchair_boarding".split(',');
        ...
        map_select_nodes.on('select', event => {
            ...
            if (this.selected_action == 'remove'){
                this.deleteNodeByID(
                    event.selected[0].parent.getID());
                return;
            }

            if (this.selected_action == 'select')
                this.map.getOverlayById('popup_node_info').setPosition(
        ...
        this.map = new Map({ // FIXME include in a function
            interactions: defaultsInteractions().extend([map_select_nodes, map_translate]),
        ...
        this.map.on('click', (event) => {
            if ( // this.selected_action == 'select' || // FIXME remove TEMP
                this.selected_action == 'remove'){
